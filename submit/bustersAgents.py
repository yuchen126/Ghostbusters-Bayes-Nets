...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostBeliefs in livingGhostPositionDistributions:
            prob = 0
            ghostPos = None
            for newPos, newProb in ghostBeliefs.items():
                if newProb > prob:
                    ghostPos = newPos
                    prob = newProb
            list.append(ghostPos)

        closestGhost = None
        minDist = sys.maxint
        for ghostPos in list:
        ...
        for action in legal:
            ...
            disToGhost = self.distancer.getDistance(successorPosition, closestGhost)
            if (disToGhost < minDisToGhost):
