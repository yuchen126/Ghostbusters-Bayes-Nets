...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        current position, use this line of code:
          ...
          #newPosDist[successorPosition] = probability
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, newProb in newPosDist.items():
                allPossible[newPos] = allPossible[newPos] + newProb * self.beliefs[p]
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        For each particle in the list you would calculate the probability of the ghost
        being at that location and use that to create your probability distribution.
        Then sample the correct number of particles to recreate your list
        ...
        if noisyDistance == None:
            ...
            for particle in self.particles:
                ...
                if emissionModel[trueDistance] > 0:
                    newBeliefs[particle] = newBeliefs[particle] + emissionModel[trueDistance]  #update beliefs
            ...
            if newBeliefs.totalCount() == 0:
                ...
                self.particles = []
                for i in range(self.numParticles):
                    self.particles.append(util.sample(newBeliefs))
    ...
    def elapseTime(self, gameState):
        ...
        for newParticle in newParticles:
            self.particles.append(util.sample(newParticle))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        posPermutations = list(itertools.product(positions, repeat = self.numGhosts))
        random.shuffle(posPermutations)
        for pos in posPermutations:
            particlePerPos = self.numParticles/len(posPermutations)
        ...
        for particle in self.particles:
            ...
            for ghostIndex in range(self.numGhosts):
                if noisyDistances[ghostIndex] == None:
                    ...
                    dist = util.manhattanDistance(particle[ghostIndex], pacmanPosition)
                    prob = prob * emissionModels[ghostIndex][dist]
